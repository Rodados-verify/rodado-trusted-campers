...
export const VehiculoContactModal = ({ vendedor, vehicleName }: VehiculoContactModalProps) => {
  ...
  const whatsappUrl = vendedor.telefono
    ? `https://wa.me/34${vendedor.telefono.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola, me interesa tu ${vehicleName} publicado en Rodado.`)}`
    : null;

  const phoneUrl = vendedor.telefono ? `tel:+34${vendedor.telefono.replace(/\D/g, "")}` : null;
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setOpen(false)}>
          <div className="relative w-full max-w-sm rounded-2xl border border-border bg-white p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-6 space-y-3">
              ...
              {phoneUrl && (
                <a href={phoneUrl} className="flex items-center gap-3 rounded-xl border border-border p-4 transition-colors hover:bg-muted/30">
                  ...
                  <div>
                    <p className="text-sm font-semibold">Llamar</p>
                    <p className="text-xs text-muted-foreground">{vendedor.telefono}</p>
                  </div>
                </a>
              )}
              <a href={emailUrl} className="flex items-center gap-3 rounded-xl border border-border p-4 transition-colors hover:bg-muted/30">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-ocre/10 text-ocre">
