...
const navItems = [
  { label: "Mis encargos", path: "/taller", icon: ClipboardList },
  { label: "Mi perfil", path: "/taller/perfil", icon: User },
];

const TallerLayout = ({ children }: { children: ReactNode }) => {
  const { user, signOut } = useAuth();
  const location = useLocation();
  const isMobile = useIsMobile();

  const isActive = (path: string) => {
    if (path === "/taller") return location.pathname === "/taller" || location.pathname.startsWith("/taller/encargo/");
      ...
      <div className="flex min-h-screen flex-col bg-background">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-30 flex h-16 items-center justify-around border-t border-border bg-background/95 backdrop-blur">
          {navItems.map(item => {
            ...
            return (
              <Link key={item.path} to={item.path} className={cn("flex flex-col items-center gap-0.5 px-3 py-1 text-xs transition-colors", active ? "text-ocre" : "text-muted-foreground")}>
    ...
    <div className="flex min-h-screen">
      <aside className="fixed left-0 top-0 z-30 flex h-screen w-64 flex-col bg-forest text-white">
        <div className="flex h-16 items-center px-6">
          ...
          <span className="ml-2 rounded bg-white/20 px-2 py-0.5 text-xs font-medium">Taller</span>
        </div>
        <nav className="flex-1 space-y-1 px-3 py-4">
          {navItems.map(item => {
            const active = isActive(item.path);
            return (
              <Link key={item.path} to={item.path} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors", active ? "bg-ocre text-white" : "text-white/70 hover:bg-white/10 hover:text-white")}>
